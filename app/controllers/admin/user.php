...
class user extends front_base {
	...
	public function my_buy_finish() {
	    ...
	    $bid_list_01 = $this->get_sale_list($userData['userid'], '입찰', '', '낙찰');
	    $bid_list_02 = $this->get_sale_list($userData['userid'], '입찰', '', '유찰');
	...
	public function my_sale_wait() {
	    ...
	    $bid_list = $this->get_sale_list($userData['userid'], '입찰', 'wait');
	...
	public function my_sale_reply() {
	    ...
	    $this->template->assign('qna_list', $qna_map['qna_list']);
	    $this->template->assign('qna_cnt', $qna_map['qna_cnt']);
	...
	public function my_sale_ing() {
	    ...
	    $bid_list = $this->get_sale_list($userData['userid'], '입찰', 'ing');
	...
	public function my_sale_finish() {
	    ...
	    $bid_list = $this->get_sale_list($userData['userid'], '입찰', 'finish');
	...
	public function my_sale_estimate() {
	    ...
	    $query = "select *, a.userid as sale_userid, c.userid as dealer_userid from fm_cm_machine_sales a, fm_cm_machine_sales_info b, fm_cm_machine_estimate_dealer c ".
	   	    "where a.sales_seq = b.sales_seq and b.info_seq = c.info_seq and c.state = '작성완료' and c.userid = '".$userData['userid']."' order by c.reg_date desc";
	    $query = $this->db->query($query);
	...
	private function get_proposal_list_for_sale($userid) {
	    $resultMap = array();
	    $query = "select count(*) as cnt from fm_cm_machine_proposal a, fm_cm_machine_sales_info b, fm_cm_machine_sales c where a.info_seq = b.info_seq and b.sales_seq = c.sales_seq and c.userid = '".$userid."' order by reg_date desc";
	    $query = $this->db->query($query);
	    $result = $query->row_array();
	...
	private function get_sales_count($userid) {
	    $result_array = array();
	    $query = "select count(*) as fixed_cnt from fm_cm_machine_sales a, fm_cm_machine_sales_info b, fm_cm_machine_sales_detail c where a.sales_seq = b.sales_seq and b.info_seq = c.info_seq and b.state = '승인' and sales_yn = 'n' and method = '고정가격판매' and userid = '".$userid."'";
	    $query = $this->db->query($query);
	    $result = $query->row_array();
	    $result_array['fixed_cnt'] = $result['fixed_cnt'];
	    
	    $query = "select count(*) as bid_cnt from fm_cm_machine_sales a, fm_cm_machine_sales_info b, fm_cm_machine_sales_detail c where a.sales_seq = b.sales_seq and b.info_seq = c.info_seq and sales_yn = 'n' and method = '입찰' and userid = '".$userid."'";
	    $query = $this->db->query($query);
	    $result = $query->row_array();
	    $result_array['bid_cnt'] = $result['bid_cnt'];
	    
	    $query = "select count(*) as direct_cnt from fm_cm_machine_sales a, fm_cm_machine_sales_info b where a.sales_seq = b.sales_seq and sales_yn = 'n' and type = 'direct' and userid = '".$userid."'";
	    $query = $this->db->query($query);
	    $result = $query->row_array();
	    $result_array['direct_cnt'] = $result['direct_cnt'];
	    
	    $query = "select count(*) as complete_cnt from fm_cm_machine_sales a, fm_cm_machine_sales_info b where a.sales_seq = b.sales_seq and sales_yn = 'y' and userid = '".$userid."'";
	    $query = $this->db->query($query);
	    $result = $query->row_array();
	...
	private function get_osc_count($userid) {
	    $result_array = array();
	    $query = "select count(*) as osc_reg_cnt from fm_cm_machine_outsourcing where permit_yn = 'y' and state = 1 and userid = '".$userid."'";
	    $query = $this->db->query($query);
	    $result = $query->row_array();
	    $result_array['osc_reg_cnt'] = $result['osc_reg_cnt'];
	    
	    $query = "select count(*) as osc_ing_cnt from fm_cm_machine_outsourcing where permit_yn = 'y' and state = 2 and userid = '".$userid."'";
	    $query = $this->db->query($query);
	    $result = $query->row_array();
	    ...
	    $query = "select count(*) as ptn_wait_cnt from fm_cm_machine_partner a, fm_cm_machine_partner_osc b where a.partner_seq = b.partner_seq and state = 0 and userid = '".$userid."'";
	    $query = $this->db->query($query);
	    $result = $query->row_array();
	    $result_array['ptn_wait_cnt'] = $result['ptn_wait_cnt'];
	    
	    $query = "select count(*) as ptn_ing_cnt from fm_cm_machine_partner a, fm_cm_machine_partner_osc b where a.partner_seq = b.partner_seq and state > 0 and state < 3 and userid = '".$userid."'";
	    $query = $this->db->query($query);
	    $result = $query->row_array();
	...
	private function get_sale_list($userid, $type, $state, $sub) {
	    if($type == '다이렉트') {
	    ...
	    if($type == '셀프판매') {
	        ...
	        $where_query .= "and b.state != '승인' and (b.sort_price is null or b.sort_price = 0) and sales_yn = 'n' ";
	        if($type == '입찰') 
	            $detail_query .= "and bid_yn = 'y' ";
	    } else if ($state == 'ing') {
	        $where_query .= "and b.state = '승인' and sales_yn = 'n' and b.sort_price is not null and b.sort_price != 0 ";
	        if($type == '입찰')
	            $detail_query .= "and bid_yn = 'n' ";
	    } else if ($state == 'finish') {
	        $where_query .= "and b.state = '승인' and sales_yn = 'y' and b.sort_price is not null and b.sort_price != 0 ";
	    } else {
	        $where_query .= "and b.state = '승인' and b.sort_price is not null and b.sort_price != 0 ";
	    }
	    if(!empty($sub)) {
	        if($sub == 'buy') {
	    ...
	    if(!empty($recent_cookies)) {
	        foreach($recent_cookies as $row) {
	            if(!empty($row)) {
                    ...
                    $query2 = "select count(*) as apply_cnt from fm_cm_machine_partner_osc where osc_seq = ".$row;
                    $query2 = $this->db->query($query2);
	...
	private function get_ptn_list($userid, $state) {
	    if($state == 'wait') {
	        $where_query = "and d.state = 0";
	    } else if ($state == 'ing') {
	        $where_query = "and d.state in(1, 2)";
	    }  else if ($state == 'finish') {
	...
	private function get_my_new($userid) {
        ...
        $query = "select * from fm_cm_machine_proposal a, fm_cm_machine_sales_info b, fm_cm_machine_sales c where a.info_seq = b.info_seq and b.sales_seq = c.sales_seq and a.permit_yn = 'h' and c.userid = '".$userid."'";
        $query = $this->db->query($query);
        $result = $query->result_array();
        if(count($result) > 0)
        ...
        $query = "select * from fm_cm_machine_question a, fm_cm_machine_sales_info b, fm_cm_machine_sales c where a.info_seq = b.info_seq and b.sales_seq = c.sales_seq and view_yn = 'n' and a.userid = '".$userid."'";
        $query = $this->db->query($query);
        $result = $query->result_array();
        if(count($result) > 0)
