...
class batch_process extends admin_base {
	...
	public function set_payments_new($params, $sc){
		...
		if($sc[$sc['callPage']] <= 0){
			$callback = "parent.location.reload();";
			openDialogAlert("마일리지 또는 포인트 항목은 필수 입니다.", 400, 140, 'parent', $callback);
			exit;
		}
		...
		$callback = "parent.location.reload();";
		openDialogAlert($totalCnt."명에게 마일리지가 지급 되었습니다.", 400, 140, 'parent', $callback);
		exit;
	}
	...
	public function set_payments_batch($params, $sc){
		...
		$setData = array(
			...
			'count'			=> $params['mcount'],
			'file_name'		=> $params['memo']."|".$params[$sc['callPage']],
			'state'			=> 0,
			'limit_count'	=> $sc['limitCount'],
			'reg_date'		=> $regDate
		);

		$this->db->insert('queue', $setData);
		...
		if( $queueID > 0 ){
			...
			*/
			...
			$callback = "parent.location.reload();";
			openDialogAlert("지급 요청 완료 (회원 > 마일리지/포인트 지급내역에서 확인 가능)", 400, 140, 'parent', $callback);
			exit;
		} else {
	...
	public function send_sms(){
		...
		// 파일로깅
		$this->smsmodel->sms_log_write($_POST['send_message']);
		...
		if ($_POST['sms_reserve_yn'] == 'y') {
			...
			$this->sms_reserve = $date." ".$_POST['reserve_hour'].":".$_POST['reserve_min'].":00";
		}
	...
	public function send_email(){
		...
		if($_POST["send_to"] && $_POST["send_to"] != "메일 주소를 입력하세요"){
		...
		$bodys = adjustEditorImages($_POST['contents']);
	...
	**/
	...
	public function restock_notify_send_sms(){
		...
		$callback = 'parent.closeDialog("processDiv");';
		if($_POST['limit_goods_name_yn'] == "y"){
			if($_POST['limit_goods_name'] == ""){
				openDialogAlert("상품명 길이제한을 입력해주세요.",400,140,'parent',$callback);
				exit;
			}
		}

		if($_POST['limit_goods_option_yn'] == "y"){
			if($_POST['limit_goods_option'] == ""){
		...
		if($targetList){
			...
			foreach($targetList as $goods_seq=>$v){
				...
				foreach($v['phone'] as $cnt=>$cellphone){
					...
					if($_POST['limit_goods_option_yn'] == "y") $v['option'][$x] = getstrcut(strip_tags($v['option'][$x]),$_POST['limit_goods_option']);
					...
					if($_POST['shorten_url_yn'] == "y"){
						$goods_url	= urlencode($goods_url."/goods/view?no=".$goods_seq);
						$goods_url2	= get_shortURL($goods_url);
						//debug($goods_url2);
						## 짧은 URL 오류시 긴 URL로 대체
						if($goods_url2 == "INVALID_APIKEY"){
							$goods_url2 = urldecode($goods_url);
						}
					}else{
	...
	public function send_email_dormancy(){
		...
		$this->config_basic['domain'] = ($this->config_system['domain']) ? "http://".$this->config_system['domain'] : "http://".$CI->config_system['subDomain'];
