...
class sch extends front_base
{
    ...
    public function mch_detail($type, $info_seq, $est)
    {
        ...
        $other_model = $est == 'est' ? $resultMap['other_model_est'] : $resultMap['other_model'];
        ...
        $this->template->assign('other_model', $other_model);
    ...
    public function find_reg_process()
    {
        ...
        for ($i = 0; $i < $find_cnt; $i ++) {
            ...
            $find_seq = $this->db->insert_id();
            
            $query = "select * from fm_cm_machine_find a, fm_cm_machine_kind b where find_seq = ".$find_seq;
            $query = $this->db->query($query);
            $result = $query->row_array();
            ...
            $data = array(
                ...
                'model_name' => $model_name_arr[$i],
                'mnf_name' => $mnf_name_arr[$i],
                'service_list' => $deliver_service_arr[$i],
                'pay_price' => 0,
                'pay_state' => '승인대기',
                'deliv_state' => '작업시작전',
            );
            if($deliver_service_arr[$i] != '신청안함')
                $this->db->insert('fm_cm_machine_delivery', $data);
        }
    ...
    private function get_search_result($type, $cate_k, $cate_t, $cate_f, $cate_m, $cate_y, $cate_p, $cate_a, $h, $d, $o, $more)
    {
        ...
        if ($cate_f != '0') {
            if($cate_t == '0') {
                ...
                $query = $this->db->query($query);
                $result = $query->row_array();
                $where_query .= "and h.mnf_name='" . $result['mnf_name'] . "' ";
            } else {
        ...
        if ($type == 'c') {
            $query = "select * from fm_cm_machine_sales a, fm_cm_machine_sales_info b, fm_cm_machine_kind c, fm_cm_machine_sales_picture d, fm_cm_machine_area e, fm_cm_machine_sales_advertise f, fm_cm_machine_model g, fm_cm_machine_manufacturer h" . $ad_from_query . " " . "where a.sales_seq = b.sales_seq and b.kind_seq = c.kind_seq and b.info_seq = d.info_seq and b.area_seq = e.area_seq and b.info_seq = f.info_seq and b.model_seq = g.model_seq and b.mnf_seq = h.mnf_seq " . "and b.state = '승인' and d.sort = 2 and b.sort_price is not null and f.ad_name = '자동 업데이트' and f.update_time is not null " . $where_query . ' ' . "order by update_time desc";
            $query = $this->db->query($query);
    ...
    private function get_detail_query($type, $info_seq)
    {
        ...
        $hope_price = $result[0]['hope_price'];
        ...
        $resultMap['other_model_est'] = $this->get_other_model_price($model_seq, $hope_price);
        ...
        $query = "select count(*) as sale_ing_cnt from fm_cm_machine_sales a, fm_cm_machine_sales_info b where a.sales_seq = b.sales_seq and b.state = '승인' and b.sales_yn = 'n' and b.sort_price is not null and b.sort_price != 0 and a.userid = '" . $userid . "'";
        $query = $this->db->query($query);
        $resultMap['sale_ing_cnt'] = $query->row()->sale_ing_cnt;

        $query = "select count(*) as sale_finish_cnt from fm_cm_machine_sales a, fm_cm_machine_sales_info b where a.sales_seq = b.sales_seq and b.state = '승인' and b.sales_yn = 'y' and b.sort_price is not null and b.sort_price != 0 and a.userid = '" . $userid . "'";
        $query = $this->db->query($query);
