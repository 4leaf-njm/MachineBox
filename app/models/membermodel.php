...
class Membermodel extends CI_Model {
	...
	public function get_member_data($seq){
		...
		if(!$member_address['user_name']  || !$member_address['cellphone'] || !$member_address['zipcode']){
	...
	public function admin_member_list($sc) {
		...
		if( !empty($sc['lastlogin_sdate']) && !empty($sc['lastlogin_edate']) ){
			if(!$sc['dormancy']){
				...
				$sqlWhereClause .= " AND ((A.lastlogin_date between '{$sc['lastlogin_sdate']}{$add_stime}' and '{$sc['lastlogin_edate']}{$add_etime}') or ( A.lastlogin_date = '0000-00-00 00:00:00' and (A.regist_date between '{$sc['lastlogin_sdate']}{$add_stime}' and '{$sc['lastlogin_edate']}{$add_etime}'))) ";
			}
			...
			if(!$sc['dormancy']){
				...
				$sqlWhereClause .= " AND ((A.lastlogin_date >= '{$sc['lastlogin_sdate']}{$add_stime}') or (A.lastlogin_date = '0000-00-00 00:00:00' && A.regist_date >= '{$sc['lastlogin_sdate']}{$add_stime}')) ";
			}
		}else if( empty($sc['lastlogin_sdate']) && !empty($sc['lastlogin_edate']) ){
			if(!$sc['dormancy']){
				if($sc['lastlogin_search_type'] == 'out'){
				...
				$sqlWhereClause .= " AND ((A.lastlogin_date <= '{$sc['lastlogin_edate']}{$add_stime}') or (A.lastlogin_date = '0000-00-00 00:00:00' && A.regist_date <= '{$sc['lastlogin_edate']}{$add_stime}')) ";
			}
		}
		###테스트 아이디 검색조건 추가
		...
		if($sc['pageType'] == "search"){
			...
			$dormancyquery = $this->db->query($dormancy_query);
		...
		# 회원등급별 회원수
		$data['grade_cnt'] = $this->get_grade_member_cnt($sqlFromClause,$sqlWhereClause);
	...
	public function admin_member_list_spout($sc) {
		...
		### 가입승인			
		if( !empty($sc['status']) ){ 
			$sqlWhereClause = " WHERE B.bpermit_yn != 'y'";
		...
		if($sc['dealer'] == 'y') {
		    $sqlWhereClause .= " AND B.main_dealer_yn != 'n'";
		}
		
		if($sc['main_dealer'] == 'y') {
		    $sqlWhereClause .= " AND B.main_dealer_yn = 'y'";
		}
		    
		//18.02.28 kmj 선택 다운로드 일때만 적용 되도록 수정
		if($sc['excel_type'] == "select" && $sc['excel_spout_query']){
			$members = join("','", $sc['member_chk']);   
			$sqlWhereClause .= " AND A.member_seq IN ('{$members}')";
		}

		###
		$sc['keyword'] = addslashes($sc['keyword']);
		if( !empty($sc['keyword']) || strlen($sc['keyword'])){
			$sqlWhereClause .= " and ( A.userid like '%".$sc['keyword']."%' or A.user_name like '%".$sc['keyword']."%' or AES_DECRYPT(UNHEX(A.email), '{$key}') like '%".$sc['keyword']."%' or AES_DECRYPT(UNHEX(A.phone), '{$key}') like '%".$sc['keyword']."%' or AES_DECRYPT(UNHEX(A.cellphone), '{$key}') like '%".$sc['keyword']."%' or A.address like '%".$sc['keyword']."%' or A.address_detail like '%".$sc['keyword']."%' or A.nickname like '%".$sc['keyword']."%' or A.sns_n REGEXP '".str_replace("*",".",substr($sc['keyword'],0,-1))."' )";
		}
		...
		if($snssqlWhereClause){
			$sqlWhereClause .= " AND (".implode(" OR ", $snssqlWhereClause)." ) ";
		}
		...
		foreach($data['result'] as &$row) {
		    ...
		    $row['main_dealer_yn'] = $result['main_dealer_yn'];
		}
		...
		if($sc['pageType'] == "search"){
			...
			$dormancy_query = 'select count(A.member_seq) as cnt '. $sqlFromClause . $sqlWhereClause.$whereDormancy;
			$dormancyquery = $this->db->query($dormancy_query);
			$dormancyrow = $dormancyquery->result_array();
			$data['dormancy_count'] = $dormancyrow[0]['cnt'];
		}
	...
	public function admin_withdrawal_list($sc) {
		...
		if( !empty($sc['sdate']) && !empty($sc['edate']))
		{
			$sql .= " AND date_format(A.regist_date, '%Y-%m-%d') >= '{$sc['sdate']}' AND date_format(A.regist_date, '%Y-%m-%d') <= '{$sc['edate']}' ";
		}
	...
	public function get_sms_restriction(){
		...
		$restriction_title = array("comm"				=> "공통"
									...
									,"coupon_cancel"	=> "환불완료 (취소)"
									,"coupon_refund"	=> "환불완료 (반품)"
								);
	...
	public function email_unsubscribe($email){
		...
		$key = get_shop_key();
		$sql = "select member_seq from fm_member where AES_DECRYPT(UNHEX(email), '{$key}')=?";
		...
		if(count($result) == 0){
			...
			foreach($result as $data){
				$upSql = "update fm_member set mailing = 'n' where member_seq = '".$data['member_seq']."'";
				$this->db->query($upSql);
			}
			$msg = "수신거부 처리되었습니다.";
		}
	...
	public function dormancy_off($member_seq = null){
		...
		if($dormancy_seq){
			...
			$mem_select = 'user_name,email,phone,cellphone,zipcode,address_type,address,address_street,address_detail,birthday,auth_code,auth_vno,auth_type,status';
	...
	# 그룹별 회원수
	public function get_grade_member_cnt($sqlFromClause,$sqlWhereClause){
	...
	public function set_confirm_update($memberArr){
		if(is_array($memberArr)){
			...
			$sql = "update fm_member a, fm_member_business b set bpermit_yn='y', admin_log=concat('".$admin_memo."',admin_log) where a.member_seq = b.member_seq and b.member_seq in(".implode(",",$memberArr).")";
			$this->db->query($sql);
		}
